...
import '../model/received_order_detail.dart';

class MyReceivedOrderDetailScreen extends StatefulWidget {
  final int id;

  const MyReceivedOrderDetailScreen({
    super.key,
    required this.id,
  });

  @override
  State<MyReceivedOrderDetailScreen> createState() =>
      _MyPurchaseHistoryDetailScreenState();
}

class _MyPurchaseHistoryDetailScreenState
    extends State<MyReceivedOrderDetailScreen> {
  ReceivedOrderDetailData? data;
  ...
  getData() async {
    ...
    ApiClient().request(
      ...
      callback: (response, success) {
        ViewUtil.hideLoader();
        if (success) {
          data = ReceivedOrderDetailResponse.fromJson(response?.data).data;
          setState(() {});
        }
      },
    );
  ...
  updateOrderStatus(String status) async {
    ...
    await ApiClient().request(
      url: "seller/order/${widget.id}/status-update",
      method: Method.POST,
      params: {
        "status": status,
      },
      callback: (response, success) async {
        ViewUtil.hideLoader();
        if (success) {
          final resp = GlobalResponse.fromJson(response?.data);
          ViewUtil.snackBar(resp.message ?? "", context);
  ...
  Widget build(BuildContext context) {
    return TixeMainScaffold(
      ...
      bottomNavigationBar: data?.status?.toLowerCase().trim() == "ongoing"
          ? _onGoingOrderButtons()
          : data?.status?.toLowerCase().trim() == "pending"
              ? _pendingOrderButton()
              : null,
    );
  }
  ...
  Widget _addressSection(String title, OrderAddress? address) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
